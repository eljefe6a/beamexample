...
public class InjectorIterator implements Iterator<GameActionInfo> {
  ...
  public static class SourceConfig implements Serializable {
    ...
    /**
     * Divide a SourceConfig into some number of parts, with the following constraints:
     *
     * 1. Each part should be responsible for an integer number of teams.
     * 2. Entries and QPS should be divided between splits proportionally to the teams.
     */
    public SourceConfig[] split(int numParts) {
      ...
      for (int i = 0; i < numParts; i++) {
        ...
        parts[i] = new SourceConfig(
            numEntries == null ? null : newTeams * (numEntries / numTeams),
            newTeams,
            newTeams * (minQps / numTeams),
            newTeams * (maxQps / numTeams));
      }
      return parts;
    }
  }
  ...
  public GameActionInfo next() {
    ...
    GameActionInfo nextItem = new GameActionInfo(
        team.getRandomUser(random),
        team.teamName,
  ...
  /**
   * Select a random team from the list of teams. If the selected team is too old w.r.t its
   * expiration remove it, replacing it with a new team.
   */
  ...
  private TeamInfo createTeam(Instant now) {
    ...
    String robot = random.nextInt(ROBOT_PROBABILITY) == 0
        ? null : "Robot-" + random.nextInt(NUM_ROBOTS);
  ...
  private static class TeamInfo {
    ...
    @Nullable private String robot;
    private int numMembers;

    private TeamInfo(
        String teamName, Instant startTime, Instant endTime, @Nullable String robot, 
        int numMembers) {
    ...
    String getRandomUser(Random random) {
      // If we have a robot, then the probability of generating the robot should be
  ...
  private static class QpsIterator implements Iterator<Instant> {
    ...
    public QpsIterator(Random random, Instant initialTimestamp, int minQps, int maxQps) {
      Preconditions.checkArgument(minQps <= maxQps, 
          "minQps(%1) should be <= maxQps(%2)", minQps, maxQps);
      this.random = random;
